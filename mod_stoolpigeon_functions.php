		...
		elseif (strpos($line, '='))
		{
			...
			if ($catch_quotes == '1')
			{
				...
				if ($trimmed_value{0} == '"' && $trimmed_value{$trimmed_len} == '"')
				{
					...
					//Due now is allowed use escaped quotes this one will help to not throw an error when used.
					$cleaned = preg_replace('/\\"/', 'MOD_CATCHED_ESCAPED_QUOTES', $cleaned);
					//Now "$cleaned" have out the escaped quotes and only other wrong double quotes usage will be under revison.
